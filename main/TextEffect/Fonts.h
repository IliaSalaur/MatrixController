...
namespace FontData
{
  const uint8_t font5x7[][5] = {
    ...
    // {0x0, 0b0000000, 0b0001000, 0b1111100, 0x0},    // 1
    ...
    {0b0001100, 0b10010000, 0b11010000, 0b10110000, 0b0011100}, // 0x79 y
  ...
  const uint8_t  font5x7rus[][5] = 
    {
    {0x7e, 0x49 | 2, 0b1001010, 0x49 | 2, 0b1001010}, //Ё 
    ...
    {0x7c, 0x21, 0x12, 0x09, 0x7c}, //й  0xA9
    ...
    {0x38, 0x54 | 0b0000010, 0x54, 0x54 | 0b0000010, 0x18} //ё
  };

  const int font5x7rus_charsNum[]
  {
    53377,
    53392,
    53393,
...
{
protected:
  const uint8_t (*_chars)[_fontW];
  const size_t numberOfChars = _charNum;
public:
  virtual const uint8_t* getBytes(int index) = 0;
...
public:
  Font5x7(const uint8_t (*arr)[5] = FontData::font5x7)
  {
    _chars = arr;
  }

  const uint8_t* getBytes(int index) override
  {
    if(index <= 127)
    {
      return _chars[index];
...
class Font5x7Rus : public IFont<66, 5, 7>
{
private:
  const int *_charsNum;
public:
  Font5x7Rus(const uint8_t (*arr)[5] = FontData::font5x7rus, const int* charNum = FontData::font5x7rus_charsNum)
  {
    _chars = arr;
    _charsNum = charNum;
  }
...
class Font3x5Digits : public IFont<13, 3, 5>
{
private:

public:
  Font3x5Digits(const uint8_t (*arr)[3])
  {
    _chars = arr;
  }
